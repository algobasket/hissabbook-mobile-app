  ...
  return (
    <ProtectedRoute>
      <AppShell activePath="/cashbooks">
        <div className="mx-auto max-w-4xl space-y-4 md:space-y-6 px-3 md:px-6 py-4 md:py-10">
          {/* Header */}
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <button
              onClick={() => router.push("/cashbooks")}
              className="flex items-center gap-2 text-xs sm:text-sm font-medium text-slate-600 hover:text-[#2f4bff] transition self-start"
            >
                ...
                <>
                  {hasPermissionCheck("cashbooks.update") && (
                    <button
                      ...
                      className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 sm:px-4 sm:py-2 text-xs sm:text-sm font-medium text-slate-700 hover:bg-slate-50"
                    >
                      Rename Book
                    </button>
                  )}
                  ...
                  {hasPermissionCheck("cashbooks.delete") && (
                    <button
                      ...
                      className="rounded-lg border border-red-200 bg-white px-3 py-1.5 sm:px-4 sm:py-2 text-xs sm:text-sm font-medium text-red-600 hover:bg-red-50"
                    >
                      Delete Book
                    </button>
                  )}
          ...
          {isManagerOrAdmin() && (
            <div className="rounded-2xl border border-slate-200 bg-white p-4 md:p-6">
              <div className="mb-4">
                <h2 className="text-base md:text-lg font-semibold text-[#1f2937]">Staff Permissions</h2>
                <p className="mt-1 text-xs md:text-sm text-slate-500">Control whether staff members can create Cash In and Cash Out entries.</p>
              </div>
              
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 rounded-xl border border-slate-200 bg-slate-50 p-3 md:p-4">
                <div className="flex-1 min-w-0">
                  <h3 className="text-sm md:text-base font-medium text-[#1f2937]">Allow Staff to Create Cash In/Cash Out</h3>
                ...
                <label className="relative inline-flex cursor-pointer items-center flex-shrink-0 sm:ml-4">
                  <input
                    type="checkbox"
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4 md:p-6">
            <div className="mb-4 md:mb-6">
              <h2 className="text-base md:text-lg font-semibold text-[#1f2937]">Members</h2>
              <p className="mt-1 text-xs md:text-sm text-slate-500">Add, Change role, Remove.</p>
            </div>

            {/* Add Members Section - Show based on permissions */}
            {hasPermissionCheck("cashbooks.update") && (
              <div className="mb-4 md:mb-6 rounded-xl border border-slate-200 bg-slate-50 p-4 md:p-6">
                <h3 className="mb-2 text-sm md:text-base font-semibold text-[#1f2937]">Add Members</h3>
                <p className="mb-4 text-xs md:text-sm text-slate-600">
                  Manage your cashflow together with your business partners, family or friends by adding them as members.
                </p>
                <button
                  onClick={handleAddMemberClick}
                  className="flex items-center justify-center gap-2 rounded-xl bg-[#2f4bff] px-4 py-2.5 text-xs md:text-sm font-semibold text-white shadow-sm hover:bg-[#2f4bff]/90 transition w-full sm:w-auto"
                >
            ...
            <div className="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
              <h3 className="text-sm md:text-base font-semibold text-[#1f2937]">Total Members ({members.length})</h3>
              <button 
                onClick={handleViewRolesClick}
                className="text-xs md:text-sm font-medium text-[#2f4bff] hover:underline self-start sm:self-auto"
              >
              ...
              <div className="space-y-3">
                {members.map((member) => {
                  ...
                  return (
                    <div
                      key={member.id}
                      className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 rounded-xl border border-slate-200 bg-white p-3 md:p-4 hover:shadow-sm transition"
                    >
                      <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
                        <div
                          className={`flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-full text-white font-semibold flex-shrink-0 ${getAvatarColor(
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
                            <p className="text-sm sm:text-base font-semibold text-[#1f2937] truncate">
                              {isCurrentUser ? "You" : member.name || member.email}
                            </p>
                            {member.phone && (
                              <span className="text-xs sm:text-sm text-slate-500">{member.phone}</span>
                            )}
                          </div>
                          {member.email && (
                            <p className="mt-1 text-xs sm:text-sm text-slate-500 truncate">{member.email}</p>
                          )}
                        </div>
                      </div>
                      <div className="flex items-center justify-between sm:justify-end gap-2 sm:gap-3">
                        <span
                          className={`rounded-lg border px-2 py-1 sm:px-3 text-xs font-semibold ${getRoleColor(role)} whitespace-nowrap`}
                        >
                          {role}
                        </span>
                        {!isCurrentUser && !member.isOwner && member.id !== book?.ownerId && hasPermissionCheck("cashbooks.update") && (
                          <button
                            ...
                            className="rounded-lg p-1.5 sm:p-2 text-slate-400 hover:bg-slate-100 hover:text-red-600 transition flex-shrink-0"
                            title="Remove member"
                          >
