...
const statCardGradients = {
  wallet: "from-[#10B981] via-[#34D399] to-[#6EE7B7]",
  pending: "from-[#059669] via-[#10B981] to-[#34D399]",
  payouts: "from-[#047857] via-[#059669] to-[#10B981]",
} as const;
...
export default function DashboardContent() {
  ...
  const renderUserName = () => {
    ...
    return (
      <span className="inline-flex items-center gap-2">
        <span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-[#10B981]/10 text-[#10B981]">
          <svg
            className="h-4 w-4"
            fill="none"
            stroke="currentColor"
  ...
  if (isManagerUser) {
    ...
    return (
      <div className="flex w-full flex-col gap-6 md:gap-10">
        <section className="rounded-[32px] md:rounded-[40px] border border-white/50 bg-white/80 p-5 md:p-8 lg:p-10 shadow-[0_45px_90px_rgba(16,185,129,0.12)] backdrop-blur">
          <div className="flex flex-col gap-4 md:gap-6 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-[10px] md:text-xs font-semibold uppercase tracking-[0.4em] text-[#10B981]/90">Overview</p>
              <h1 className="mt-2 md:mt-3 text-2xl md:text-3xl font-semibold text-[#111827]">
                {renderUserName()}
              </h1>
              {displayRole && (
                <p className="mt-1 text-xs md:text-sm font-medium text-[#10B981]">
                  {displayRole}
                </p>
              )}
            ...
            <div className="flex flex-wrap gap-3">
              <button
                onClick={() => router.push("/cashbooks")}
                className="rounded-full bg-[#10B981] px-4 md:px-5 py-2 text-xs md:text-sm font-semibold text-white shadow-[0_14px_30px_rgba(16,185,129,0.25)] transition hover:-translate-y-0.5"
              >
                View Cashbooks
              </button>
            </div>
          ...
          <div className="mt-6 md:mt-8 grid gap-4 md:gap-6 md:grid-cols-2 lg:grid-cols-4">
            {dataLoading ? (
              ...
              managerStatCards.map((card) => (
                <div key={card.title} className={`rounded-[24px] md:rounded-[32px] bg-gradient-to-br ${card.gradient} p-[1px] shadow-lg`}>
                  <div className="flex h-full w-full flex-col gap-3 md:gap-4 rounded-[23px] md:rounded-[30px] bg-white/85 p-4 md:p-6">
                    <div className="flex items-center justify-between">
                      <span className="text-xs md:text-sm font-semibold text-slate-500">{card.title}</span>
                      <span className="rounded-xl md:rounded-2xl bg-white/80 px-2 py-1 md:px-3 md:py-2 text-[#10B981] text-xs md:text-sm">•</span>
                    </div>
                    <p className="text-xl md:text-2xl font-semibold text-[#111827]">{card.value}</p>
                    <span className="text-[10px] md:text-xs font-semibold uppercase tracking-[0.3em] text-slate-400">{card.meta}</span>
                  </div>
                </div>
              ))
            )}
        ...
        <div className="grid gap-10 lg:grid-cols-[1.2fr_0.8fr]">
          <section className="rounded-[36px] border border-white/50 bg-white/80 p-8 shadow-[0_30px_60px_rgba(16,185,129,0.1)] backdrop-blur">
            <div className="flex items-center justify-between">
              <div>
              ...
              <button
                onClick={() => router.push("/cashbooks")}
                className="rounded-full border border-slate-200 bg-white px-4 py-2 text-xs font-semibold text-slate-500 transition hover:border-[#10B981]/60 hover:text-[#10B981]"
              >
                View all
              </button>
            </div>
          ...
          <aside className="space-y-6">
            <div className="rounded-[32px] border border-white/50 bg-white/85 p-8 shadow-[0_30px_60px_rgba(16,185,129,0.1)] backdrop-blur">
              ...
              <div className="mt-6 space-y-4">
                {managerQuickActions.map((action) => (
                  <button
                    ...
                    className="group flex w-full items-center gap-4 rounded-2xl border border-[#10B981]/20 bg-white/90 px-5 py-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-[#10B981]/40"
                  >
                    <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#10B981]/10 text-[#10B981]">★</span>
                    <span>
                      <span className="block text-sm font-semibold text-[#111827] group-hover:text-[#10B981]">{action.title}</span>
                ...
                <div className="mt-6 space-y-3">
                  {cashbooks.slice(0, 5).map((book) => (
                    <button
                      ...
                      className="group flex w-full items-center justify-between rounded-xl border border-slate-200 bg-white px-4 py-3 text-left transition hover:border-[#10B981]/40 hover:bg-slate-50"
                    >
                      <div>
                        <p className="text-sm font-semibold text-[#111827] group-hover:text-[#10B981]">{book.name}</p>
                        {book.description && (
                          <p className="text-xs text-slate-500">{book.description}</p>
                        )}
                      </div>
                      <svg className="h-5 w-5 text-slate-400 group-hover:text-[#10B981]" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
  ...
  return (
    <div className="flex w-full flex-col gap-4 sm:gap-5 md:gap-10">
      <section className="rounded-[12px] sm:rounded-[16px] md:rounded-[40px] border border-white/50 bg-white/90 p-3.5 sm:p-5 md:p-10 shadow-[0_4px_20px_rgba(16,185,129,0.08)] backdrop-blur">
        <div className="flex flex-col gap-1.5 sm:gap-2 md:gap-6 md:flex-row md:items-center md:justify-between">
          <div className="flex-1">
            <p className="text-[9px] sm:text-[10px] md:text-xs font-semibold uppercase tracking-[0.2em] sm:tracking-[0.25em] md:tracking-[0.4em] text-[#10B981]">Overview</p>
            <h1 className="mt-1.5 sm:mt-2 md:mt-3 text-base sm:text-lg md:text-3xl font-semibold text-[#111827] leading-tight">
              {renderUserName()}
            </h1>
            {displayRole && (
              <p className="mt-1 sm:mt-1 md:mt-1.5 text-[10px] sm:text-xs md:text-sm font-medium text-[#10B981]">
                {displayRole}
              </p>
            )}
            <p className="mt-1 sm:mt-1.5 md:mt-2 text-[10px] sm:text-xs md:text-sm text-slate-600 leading-relaxed">
              Keep an eye on wallet balances, approvals and payouts all in one place.
            </p>
          </div>
          <div className="flex flex-wrap gap-2 sm:gap-3 mt-2 sm:mt-2.5 md:mt-0">
            <button className="rounded-full bg-[#10B981] px-4 sm:px-5 md:px-6 py-2 sm:py-2.5 md:py-3 text-xs sm:text-sm md:text-sm font-semibold text-white shadow-[0_4px_12px_rgba(16,185,129,0.3)] transition-all hover:shadow-[0_6px_16px_rgba(16,185,129,0.4)] active:scale-95">
              UPI Sync Connect
            </button>
          </div>
        </div>
        <div className="mt-3 sm:mt-4 md:mt-8 grid gap-2.5 sm:gap-3 md:gap-6 md:grid-cols-3">
          {staffStatCards.map((card) => (
            <div key={card.title} className={`rounded-[10px] sm:rounded-[12px] md:rounded-[32px] bg-gradient-to-br ${card.gradient} p-[1px] shadow-sm hover:shadow-md transition-shadow`}>
              <div className="flex h-full w-full flex-col gap-1.5 sm:gap-2 md:gap-4 rounded-[9px] sm:rounded-[11px] md:rounded-[30px] bg-white/90 p-3 sm:p-3.5 md:p-6">
                <div className="flex items-center justify-between">
                  <span className="text-[9px] sm:text-[10px] md:text-sm font-semibold text-slate-700">{card.title}</span>
                  <span className="rounded-full bg-white/90 px-1.5 sm:px-2 md:px-3 py-0.5 sm:py-1 md:py-2 text-[#10B981] text-[8px] sm:text-[9px] md:text-sm">•</span>
                </div>
                <p className="text-base sm:text-lg md:text-2xl font-bold text-[#111827] leading-tight mt-0.5">{card.value}</p>
                <span className="text-[7px] sm:text-[8px] md:text-xs font-semibold uppercase tracking-[0.1em] sm:tracking-[0.12em] md:tracking-[0.3em] text-slate-500 mt-0.5">{card.meta}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="space-y-3 sm:space-y-4 md:space-y-10">
        <section className="rounded-[12px] sm:rounded-[16px] md:rounded-[36px] border border-white/50 bg-white/80 p-3 sm:p-4 md:p-8 shadow-[0_20px_40px_rgba(16,185,129,0.08)] backdrop-blur">
          <div className="flex items-center justify-between mb-2.5 sm:mb-3 md:mb-0">
            <div>
              <h2 className="text-xs sm:text-sm md:text-lg font-semibold text-[#111827]">Recent activity</h2>
              <p className="text-[9px] sm:text-[10px] md:text-sm text-slate-500 mt-0.5">Latest wallet, approval and payout updates.</p>
            </div>
            <button 
              onClick={() => router.push("/approvals")}
              className="rounded-full border border-slate-200 bg-white px-2.5 sm:px-3 md:px-4 py-1 sm:py-1.5 md:py-2 text-[9px] sm:text-[10px] md:text-xs font-semibold text-slate-600 transition active:scale-95 hover:border-[#10B981]/60 hover:text-[#10B981]"
            >
              View all
            </button>
          </div>
          <div className="mt-3 sm:mt-4 md:mt-6 space-y-2 sm:space-y-2.5 md:space-y-4">
            {dataLoading ? (
              <div className="py-4 sm:py-6 md:py-8 text-center text-[10px] sm:text-xs md:text-sm text-slate-500">
              ...
              recentActivities.map((update, index) => (
                <div
                  ...
                  className="flex items-start gap-2.5 sm:gap-3 rounded-[10px] sm:rounded-xl md:rounded-3xl border border-slate-100 bg-white/95 p-2.5 sm:p-3 md:p-4 shadow-sm hover:shadow-md active:bg-slate-50 transition-all"
                >
                  <span className={`mt-0.5 sm:mt-1 inline-flex h-6 w-6 sm:h-7 sm:w-7 md:h-8 md:w-8 items-center justify-center rounded-full text-[10px] sm:text-[11px] md:text-xs font-semibold ${update.accent} flex-shrink-0`}>
                    •
                  </span>
                  <div className="flex-1 space-y-0.5 sm:space-y-1 min-w-0">
                    <p className="text-[11px] sm:text-xs md:text-sm font-semibold text-[#111827] leading-tight">{update.title}</p>
                    <p className="text-[9px] sm:text-[10px] md:text-xs text-slate-500">{update.time}</p>
                  </div>
                </div>
              ))
            ) : (
        ...
        <div className="rounded-[10px] sm:rounded-[12px] md:rounded-[32px] border border-[#10B981]/20 bg-gradient-to-br from-[#10B981]/10 to-white/95 p-3.5 sm:p-4 md:p-6 text-[10px] sm:text-xs md:text-sm text-[#059669] shadow-sm leading-relaxed">
